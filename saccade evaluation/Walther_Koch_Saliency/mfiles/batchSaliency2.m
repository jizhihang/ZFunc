...
function [salMaps,fixations] = batchSaliency2(images,numFixations,params, MyImage)
...
for f = 1:numImg
  ...
  a = load(MyImage);
  [mx my] = size(salMaps(f).data);
  salMaps.data = imresize(a.finalSal, [mx, my]);
  salMaps.data = ( salMaps.data - min(salMaps.data(:)) ) / ( max(salMaps.data(:)) - min(salMaps.data(:)));
  myParams.foaSize =  0.1667*min(salMaps(f).origImage.size(1), salMaps(f).origImage.size(2));
